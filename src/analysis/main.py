...
system = 'Sinusoid'  # 'VanDerPol', 'Lorenz' or 'Sinusoid'
...
if __name__ == "__main__":
    ...
    elif system == 'Sinusoid':
        plt.figure(figsize=(10, 6))
        plt.plot(time, y_true_np[:, 0], label='y_true', c='C0')
        plt.plot(time, y_pred_np[:, 0], '--', label='y_predicted', c='C1')

        plt.ylabel(f'y')
        plt.title('Sinusoid trajectory: true vs predicted')
        plt.xlabel('Time (s)')
        plt.legend(loc='best')
